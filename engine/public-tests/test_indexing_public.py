...
class TestIndexer_BasicInvertedIndex(unittest.TestCase):
    ...
    def test_create_index_with_stopwords_with_mwf(self):
        '''Test creating an index with stopwords and minimum word frequency = 2'''
    ...
    def test_create_index_with_document_augmentation(self):
        stopwords = set(['and', 'the'])
        
        doc_augmentation_dict = {
            1: ['How many apples in this string?'],
            2: ['Is this the second document'],
        ...
        expected_stats = {'unique_token_count': 5,
                          ...
                          'number_of_documents': 3,
                          'mean_document_length': 46/3}
        for k, v in expected_stats.items():
            self.assertEqual(stats[k], v, 
                '%s was not what was expected %d' % (k, v))
